...
class ElectricMotor(RandomComponent):
    ...
    def initialize(self, state_space, state_positions, **__):
        ...
        if any(state in self._initial_states for state in ["psi_ralpha", "psi_rbeta"]):
            ...
            #  extracted with .item()
            nominal_values_ = [
                self._initial_limits[state].item()
                if isinstance(self._initial_limits[state], np.ndarray)
                else self._initial_limits[state]
            ...
            if isinstance(self._nominal_values, dict):
                nominal_values_ = [self._nominal_values[state] for state in self._initial_states.keys()]
                nominal_values_ = np.asarray(nominal_values_)
            else:
                nominal_values_ = np.asarray(self._nominal_values)

            state_space_idx = [state_positions[state] for state in self._initial_states.keys()]
        ...
        if random_dist is not None:
            if random_dist == "uniform":
                initial_value = (upper_bound - lower_bound) * self._random_generator.uniform(
