...
class SCMLSystem(PhysicalSystem, RandomComponent):
    ...
    def __init__(self, converter, motor, load, supply, ode_solver, tau=1e-4, calc_jacobian=None):
        ...
        self._components = [
            self._supply,
            self._converter,
            self._electrical_motor,
            self._mechanical_load,
            self._ode_solver,
        ]
    ...
    def seed(self, seed=None):
        ...
        for component, sub_seed in zip(self._components, sub_seeds):
            if isinstance(component, RandomComponent):
    ...
    def _system_equation(self, t, state, u_in, **__):
        ...
        if self._system_eq_placeholder is None:
            ...
            self._system_eq_placeholder = np.concatenate((load_derivative, motor_derivative))
            self._motor_deriv_size = motor_derivative.size
            self._load_deriv_size = load_derivative.size
        else:
            motor_state = state[self._motor_ode_idx]
            self._system_eq_placeholder[: self._load_deriv_size] = self._mechanical_load.mechanical_ode(
                t, state[self._load_ode_idx], self._electrical_motor.torque(motor_state)
            ).ravel()
            self._system_eq_placeholder[self._load_deriv_size :] = self._electrical_motor.electrical_ode(
    ...
    def _system_jacobian(self, t, state, u_in, **__):
        ...
        (
            motor_jac,
            el_state_over_omega,
            torque_over_el_state,
        ) = self._electrical_motor.electrical_jacobian(motor_state, u_in, state[self._omega_ode_idx])
        torque = self._electrical_motor.torque(motor_state)
        load_jac, load_over_torque = self._mechanical_load.mechanical_jacobian(t, state[self._load_ode_idx], torque)
...
class SynchronousMotorSystem(ThreePhaseMotorSystem):
    ...
    def simulate(self, action, *_, **__):
        ...
        i_abc = list(self.dq_to_abc_space(i_dq, eps))
...
class ExternallyExcitedSynchronousMotorSystem(SynchronousMotorSystem):
    ...
    def simulate(self, action, *_, **__):
        ...
        i_abc = list(self.dq_to_abc_space(i_dq_e[:2], eps))
        eps = ode_state[self._ode_epsilon_idx] % (2 * np.pi)
        if eps > np.pi:
            eps -= 2 * np.pi

        system_state = np.concatenate((mechanical_state, [torque], i_abc, i_dq_e, u_in[:3], u_dq_e, [eps], u_sup))
        return system_state / self._limits
...
class SquirrelCageInductionMotorSystem(ThreePhaseMotorSystem):
    ...
    def _set_indices(self):
        # Docstring of superclass
        super()._set_indices()
        self._motor_ode_idx += range(
            self._motor_ode_idx[-1] + 1,
            self._motor_ode_idx[-1] + 1 + len(self._electrical_motor.FLUXES),
        )
        self._motor_ode_idx += [self._motor_ode_idx[-1] + 1]

        self._ode_currents_idx = self._motor_ode_idx[
            self._electrical_motor.I_SALPHA_IDX : self._electrical_motor.I_SBETA_IDX + 1
        ]
        self._ode_flux_idx = self._motor_ode_idx[
            self._electrical_motor.PSI_RALPHA_IDX : self._electrical_motor.PSI_RBETA_IDX + 1
        ]
...
class DoublyFedInductionMotorSystem(ThreePhaseMotorSystem):
    ...
    def __init__(self, ode_solver="scipy.ode", **kwargs):
        ...
        self.stator_voltage_high_idx = (
            self.stator_voltage_low_idx + self._converter.subsignal_voltage_space_dims[self.stator_voltage_space_idx]
        )

        self.rotor_voltage_space_idx = 1
        self.rotor_voltage_low_idx = self.stator_voltage_high_idx
        self.rotor_voltage_high_idx = (
            self.rotor_voltage_low_idx + self._converter.subsignal_voltage_space_dims[self.rotor_voltage_space_idx]
        )
    ...
    def _build_state_names(self):
        # Docstring of superclass
        names_l = self._mechanical_load.state_names + [
            "torque",
            "i_sa",
            "i_sb",
            "i_sc",
...
class SixPhaseMotorSystem(SCMLSystem):
    ...
    def dq_to_abc_space(self, dq_quantities,epsilon):
        """
        Transformation of the dq representation into abc using the electrical angle

        Args:
            dq_quantities: the properties in the dq representation like ''[i_sd, i_sq, i_sx, i_sy, i_s0+, i_s0-]''.
            epsilon: electrical rotor position

        Returns:
            The converted quantities in the abc representation like ''[i_sa1, i_sb1, i_sc1, i_sa2, i_sb2, i_sc2]''.
        """
        abc_quantity = self._electrical_motor.q_inv(dq_quantities,epsilon)
        return abc_quantity
...
class SixPhasePMSM(SixPhaseMotorSystem): 
  ...
  def _set_indices(self):
        ...
        self.TORQUE_IDX = len(self.mechanical_load.state_names)
        currents_lower = self.TORQUE_IDX + 1
        currents_upper = currents_lower + 10 # considering six stator currents in abc and four currents in dqxy
        self.CURRENTS_IDX = list(range(currents_lower, currents_upper))
        voltages_lower = currents_upper
        voltages_upper = voltages_lower + 10
        self.VOLTAGES_IDX = list(range(voltages_lower, voltages_upper))
