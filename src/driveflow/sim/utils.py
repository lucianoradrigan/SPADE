"""Small helpers shared across sim/.

Ported from ``gym_electric_motor.utils`` (GEM 3.0.3), dropping the unused top-level
``import gymnasium`` (this module never referenced gymnasium in any executed code path).
"""
...
def state_dict_to_state_array(state_dict, state_array, state_names):
    ...
    This function is mainly used in the initialization phase to map a dictionary of passed
    state_name, state_value pairs to a numpy state array with the entries of the state_dict at the
    corresponding places of the state_names.
...
def set_state_array(input_values, state_names):
    ...
    The input values can be passed as dict with state_name: value pairs or as list / ndarray. In the
    latter case the shape of the list has to fit the state_names shape and the list will just be
    returned as array. If a float is passed as input value, then this value will be set onto all
    positions of the state array equally.

    Args:
        input_values(dict(float) / list(float) / ndarray(float) / float): Values to be set onto the
            state array.
        state_names(list(str)): List containing the state names of the physical system.

    Returns:
        An initialized state array with all values passed in input values set onto the corresponding
        position in the state_names and zero otherwise.
    """
...
def update_parameter_dict(source_dict, update_dict, copy=True):
    ...
    It is similar to pythons dict.update() method. Furthermore, it assures that all keys in the
    update dictionary are already present in the source dictionary. Otherwise a KeyError is thrown.
